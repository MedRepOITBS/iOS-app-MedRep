...
{
    BOOL isDeviceiPad = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) ? YES : NO;
    CGRect frame = isDeviceiPad  ? CGRectMake(0, 0, 300, 200) : CGRectMake(0, 0, 200, 150);
    UIView * alertSquare = [[UIView alloc]initWithFrame:frame];
...
- (void) labelSetupWithTitle:(NSString*) title andText:(NSString*) text
{
    ...
    BOOL isDeviceiPad = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) ? YES : NO;
    CGRect frame = isDeviceiPad ? CGRectMake(0, 0, 270, 75) : CGRectMake(0, 0, 180, 50);
    _textLabel = [[UILabel alloc] initWithFrame:frame];
    _textLabel.center = CGPointMake(alertView.frame.size.width/2, isDeviceiPad ? 95 : 80);
...
{
    BOOL isDeviceiPad = (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad) ? YES : NO;
    CGFloat fontSize = isDeviceiPad ? (13/2*3) : 13;
    CGFloat buttonHeight = isDeviceiPad ? 35 : 30;
    
    if (hasCancelButton) {
        //default button
        _defaultButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 84, buttonHeight)];

        //cancel button
        _cancelButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 84, buttonHeight)];
        _cancelButton.backgroundColor = [UIColor clearColor];// [UIColor colorWithRed:0.792 green:0.792 blue:0.792 alpha:1];
        
        _cancelButton.center = CGPointMake((alertView.frame.size.width/4)+3+ ((isDeviceiPad) ? 20 : 0), isDeviceiPad ? 160 : 120);
        _defaultButton.center = CGPointMake((alertView.frame.size.width*3/4)-3-((isDeviceiPad) ? 20 : 0), isDeviceiPad ? 160 : 120);
        
        [_cancelButton setTitle:@"Cancel" forState:UIControlStateNormal];
        _cancelButton.titleLabel.font = [UIFont fontWithName:@"Helvetica" size:fontSize];
        ...
        _defaultButton = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, 180, buttonHeight)];
        _defaultButton.center = CGPointMake(alertView.frame.size.width/2, isDeviceiPad ? 160 : 120);
    }
    ...
    _defaultButton.titleLabel.font = [UIFont fontWithName:@"Helvetica" size:fontSize];
	[_defaultButton addTarget:self action:@selector(handleButtonTouched:) forControlEvents:UIControlEventTouchUpInside];
    [_defaultButton.layer setCornerRadius:5.0f];
...
{
    [self handleButtonTouched:self.cancelButton];
}
